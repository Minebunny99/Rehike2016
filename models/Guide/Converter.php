...
class Converter
{
    ...
    public static function getMainSection($data, $signedIn = false)
    {
        ...
        if (null != $subscriptionsItem) {
            $subscriptionsItem->guideEntryRenderer->icon->iconType =
                "MY_SUBSCRIPTIONS";
        }
        ...
        if (!$signedIn) {
            ...
            $historyItem = self::getItemByIcon(
                $secondarySection,
                "WATCH_HISTORY"
            );
        ...
        if ($signedIn) {
            if (
                isset(
                    $mainSection[count($mainSection) - 1]
            ...
            foreach ($librarySection->sectionItems as $item) {
                ...
                if (!isset($item->guideEntryRenderer)) {
                    continue;
                }

                $iconType = @$item->guideEntryRenderer->icon->iconType;

                if (isset($supportedTypes2[$iconType])) {
                    $item->guideEntryRenderer->icon->iconType =
                        $supportedTypes2[$iconType];
                    $response[] = $item;
                }
				if (isset($supportedTypes4[$iconType])) {
                    $item->guideEntryRenderer->icon->iconType =
                        $supportedTypes4[$iconType];
    ...
    public static function getBestOfYouTubeSection()
    {
        ...
        $format = ConfigManager::getConfigProp(
            "appearance.oldBestOfYouTubeIcons"
        )
    ...
    public static function getLibrarySection($data)
    {
        ...
        $title =
            $librarySection->headerEntry->guideEntryRenderer->formattedTitle
                ->simpleText;
        ...
        if (
            isset(
                $librarySection->sectionItems[
                    count($librarySection->sectionItems) - 1
    ...
    public static function getSubscriptionSection($data)
    {
        ...
        if (!isset($section)) {
            return self::buildSubscriptionsPromoSection();
        }
        ...
        if (
            isset(
                $section->items[count($section->items) - 1]->guideEntryRenderer
                    ->icon
            )
        ) {
            array_splice($section->items, count($section->items) - 1, 1);
    ...
    public static function buildSubscriptionsPromoSection()
    {
        ...
        $response->formattedTitle = (object) [
            ...
            "navigationEndpoint" => NavigationEndpoint::createEndpoint(
                "/feed/channels"
            ),
        ];
    ...
    public static function getEndSection2($data, $signedIn = false)
    {
        $response = [];
        ...
        $mainSection2 = $data->items[3]->guideSectionRenderer->items;
        $Red2Item = self::getItemByIcon($mainSection2, "YOUTUBE_ROUND");

        if (null != $Red2Item) {
            $Red2Item->guideEntryRenderer->icon->iconType = "unlimited";
            $Red2Item->guideEntryRenderer->formattedTitle->simpleText =
                "YouTube Red";
            $Red2Item->guideEntryRenderer->navigationEndpoint->commandMetadata->webCommandMetadata->url =
                "/red";
        }
        ...
        // Red item
        if (!$signedIn) {
            $mainSection2 = $data->items[5]->guideSectionRenderer->items;
            $RedItem = self::getItemByIcon($mainSection2, "YOUTUBE_ROUND");

            //stuff
            $RedItem->guideEntryRenderer->icon->iconType = "unlimited";
            $RedItem->guideEntryRenderer->formattedTitle->simpleText =
                "YouTube Red";
            $RedItem->guideEntryRenderer->navigationEndpoint->commandMetadata->webCommandMetadata->url =
                "/red";
            if (ConfigManager::getConfigProp("appearance.redButton")) {
                $response[] = $RedItem;
            }
    ...
    public static function getItemByIcon($items, $icon)
    {
        ...
        foreach ($items as $item) {
            if ($content = @$item->guideEntryRenderer) {
                // Also skip items that don't have an icon type
                if (!is_string(@$content->icon->iconType)) {
                    continue;
                }
